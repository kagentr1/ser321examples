    ...
    try {
      ...
      if (request == null) {
        ...
        if (request.length() == 0) {
          ...
          // TO DONE: Include error handling here with a correct error code and
          // a response that makes sense
		} else {
			builder.append("HTTP/1.1 400 Bad Request\n");
			builder.append("'The server could not understand the request due to invalid syntax.'\n");
			builder.append("https://developer.mozilla.org/en-US/docs/Web/HTTP/Status#client_error_responses\n");
			builder.append("Content-Type: text/html; charset=utf-8\n");
			builder.append("\n");
			builder.append("ERROR: Could not multiply. INVALID NUMBER.");
		}
		  ...
		  for (int i = 0; i < repoArray.length(); i++) {
			  ...
			  builder.append("Repository: " + repoName);
